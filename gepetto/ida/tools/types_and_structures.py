...
def list_local_types() -> dict:
    ...
    for ordinal in range(1, type_count):
        try:
            ...
            continue
    return {"ok": True, "types": locals_out}
...
def analyze_struct_detailed(name: str) -> dict:
    ...
    result = {"name": name, "type": str(tif._print()), "size": tif.get_size(), "is_udt": tif.is_udt()}
    if not tif.is_udt():
...
def get_struct_at_address(address: str, struct_name: str) -> dict:
    ...
    udt_data = ida_typeinf.udt_type_data_t()
    if not tif.get_udt_details(udt_data):
        raise ValueError("Failed to get structure details!")
    ...
    for member in udt_data:
        ...
        try:
            if member.type.is_ptr():
                if ida_ida.inf_is_64bit():
                    value = ida_bytes.get_qword(member_addr)
                    value_str = f"0x{value:016X}"
                else:
                    value = ida_bytes.get_wide_dword(member_addr)
                    value_str = f"0x{value:08X}"
            elif member_size == 1:
                value = ida_bytes.get_wide_byte(member_addr)
                value_str = f"0x{value:02X} ({value})"
            elif member_size == 2:
                value = ida_bytes.get_wide_word(member_addr)
                ...
                for i in range(min(member_size, 16)):
                    try:
                        byte_val = ida_bytes.get_wide_byte(member_addr + i)
        ...
        result["members"].append({"offset": f"0x{offset:08X}", "type": member_type, "name": member_name, "value": value_str})
    return result
